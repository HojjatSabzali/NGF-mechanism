...
nax = 18; %number of attempts for each x  %%%%%%%%%%%%%%%%%%%
nay = 18; %number of attempts for each y  %%%%%%%%%%%%%%%%%%%

% ICOR = ICOR*15/100;
% Theta = deg2rad(160) ; %Insert deg of J
%% Prepare ICOR Data

ICORF = ICORF * (62.7/195.88);

Th = 55*(pi/180); %%%%%%%%%%%%%%%%%%%% 39 digree
...
Range_ys = abs(min(ICOR(:,2)))-(0.7*abs(min(ICOR(:,2)))); %%%%%%%%%%%%%

Range_xf = abs(min(ICOR(:,1)))+(0.7*abs(min(ICOR(:,1)))); %%%%%%%%%%%
Range_yf = abs(min(ICOR(:,2)))+(0.7*abs(min(ICOR(:,2)))); %%%%%%%%%%

A = Range_xs:(Range_xf-Range_xs)/(nax-1):Range_xf; %range of x ( Investigate range 70%)
B = Range_ys:(Range_yf-Range_ys)/(nay-1):Range_yf; %range of y ( Investigate range 70%)
